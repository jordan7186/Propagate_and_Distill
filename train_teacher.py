...
def run(args):
    ...
    assert args.feature_noise == 0  # Not interested
    ...
    if args.model_config_path is not None:
        conf = get_training_config(
            args.exp_setting + args.model_config_path, args.teacher, args.dataset
        )
    ...
    """ Model init """
    model = Model(conf)  # Model(conf, args)
    ...
    if not args.compute_min_cut:
        if "MLP" not in model.model_name:
            ...
            if "LAGE" not in args.teacher and "CAGE" not in args.teacher:
        ...
        if args.save_results:
            ...
            torch.save(indices, output_dir.joinpath("idx_splits.pt"))
