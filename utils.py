...
def add_deg_norm(g, norm_type: str = "sym"):
    assert norm_type in {
        "sym",
        "row",
    }, f"Expect norm_type to be 'sym' or 'row', got {norm_type}"
    in_degs = g.in_degrees().float().clamp(min=1)
    out_degs = g.out_degrees().float().clamp(min=1)
    if norm_type == "sym":
        norm_i = torch.pow(in_degs, -0.5).to(g.device).unsqueeze(1)
        norm_j = torch.pow(out_degs, -0.5).to(g.device).unsqueeze(1)
    elif norm_type == "row":
        norm_i = torch.pow(in_degs, -1.0).to(g.device).unsqueeze(1)
        norm_j = norm_i
    g_norm = deepcopy(g)
    g_norm.norm_i = norm_i
    g_norm.norm_j = norm_j
    return g_norm, norm_i, norm_j
