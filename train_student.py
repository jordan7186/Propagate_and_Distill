...
from models import Model, Model_dw
...
def get_args():
    ...
    parser.add_argument(
        "--device", type=int, default=0, help="CUDA device, -1 means CPU"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.9,
        help="gamma for propagation, in (0, 1]",
    )
    parser.add_argument(
        "--self_coeff",
        type=float,
        default=2.0,
        help="Coefficients for self-loops during propagation",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of coefficients in propagation",
    )
    parser.add_argument(
        "--fix_train",
        action="store_true",
        help="Set to True if Prop2",
    )
...
def get_features_dw(adj, device, is_transductive, args):
    ...
    model_emb = DeepWalk(
        G, walk_length=args.dw_walk_length, num_walks=args.dw_num_walks, workers=1
    )
    model_emb.train(
        window_size=args.dw_window_size, iter=args.dw_iter, embed_size=args.dw_emb_size
    )
...
def run(args):
    ...
    set_seed(args.seed)
    # args.dw = True
    # Really check if GLNN setting is correct
    # if args.glnn:
    #     assert args.dw is False
    #     assert args.feat_distill is False
    #     assert args.adv is False

    # Figure out which mode we are running
    if args.prop_iteration > 0:
        args.is_InD = False
        args.is_PnD = True
        mode = "PnD"
    else:
    ...
    if args.exp_setting == "tran":
        output_dir = Path.cwd().joinpath(
            ...
            "transductive",
            args.dataset,
            f"{args.teacher}_{args.student}_{mode}",
            f"seed_{args.seed}",
        )
    ...
    if args.model_config_path is not None:
        conf = get_training_config(
            ...
            args.exp_setting + args.model_config_path,
            args.student,
            args.dataset,
        )  # Note: student config
    ...
    # Override settings if explicitly mentioned in args
    if "learning_rate" in args:
        conf["learning_rate"] = args.learning_rate
    if "dropout_ratio" in args:
        conf["dropout_ratio"] = args.dropout_ratio
    ...
    if args.exp_setting == "tran":
        ...
        #         except:
        #             print(
        #                 "cannot load dw_emb, now try to calculate it ...... ",
        #                 flush=True,
        #             )
        #             network_g = g.cpu()
        #             network_g = network_g.to_networkx()
        #             print("done with network_g")
        #             dw_emb = get_features_dw(
        #                 network_g, device, is_transductive=True, args=args
        #             )
        ...
        # if (
        #     args.dw
        # ):  # We need to run it every time since seed (data split) is different.
        #     # computation optimized for large datasets.
        #     if args.dataset == "ogbn-products":
        #         dw_emb_path = output_dir.joinpath(
        #             "dw_emb.pt"
        #         )  # need to include the seed in the path
        #         # subgraph
        #         # trained_grapah = dgl.node_subgraph(g, idx_obs.to(device))
        #         # network_g = trained_grapah.cpu()
        #         # network_g = network_g.to_networkx() # Bottleneck
        #         # print("done with network_g")
        #         # position_feature_obs = get_features_dw(
        #         #     network_g, device, is_transductive=True, args=args
        #         # )
        #         # torch.save(position_feature_obs, dw_emb_path)
        #         # print("save dw_emb successfully")
        #         position_feature_obs = torch.load(dw_emb_path)
        ...
        #             position_feature_list_correct_order[idx_p_f].extend(
        #                 temp_position_feature
        #             )
        ...
        #             try:
        #                 idx_cur_node_id_neighbor = idx_test_ind_neighbor_dict[
        #                     idx_cur_node_id
        #                 ]
        #                 if len(idx_cur_node_id_neighbor):
        #                     temp_position_feature = torch.mean(
        #                         position_feature_obs[idx_cur_node_id_neighbor, :], dim=0
        #                     )
        #                 else:
        #                     temp_position_feature = np.zeros(
        #                         position_feature_obs.shape[-1]
        #                     )
        #             except:
        #                 temp_position_feature = np.zeros(position_feature_obs.shape[-1])

        #             position_feature_obs[idx_cur_node_id] = torch.tensor(
        #                 temp_position_feature, dtype=torch.float32
        #             )
        ...
        #                     temp_position_feature = np.asarray(
        #                         position_feature_list_correct_order[idx_j]
        #                     )
        #                 else:
        #                     temp_position_feature += np.asarray(
        #                         position_feature_list_correct_order[idx_j]
        ...
        #             position_feature_list_correct_order[idx_cur_node_id].extend(
        #                 temp_position_feature
        #             )

        #         position_feature = torch.tensor(
        #             position_feature_list_correct_order, dtype=torch.float32
        #         ).to(device)
        #         len_position_feature = position_feature.shape[-1]
        #         feats = torch.cat([feats, position_feature], dim=1)
        ...
        #         position_feature_obs = get_features_dw(
        #             adj_obs, device, is_transductive=False, args=args
        #         ).cpu()
    ...
    """ Model init """
    if args.dw:
        # model = Model_dw(conf, args, len_position_feature)
        pass
    else:
    ...
    if args.exp_setting == "tran":
        out, score_val, score_test = distill_run_transductive(
            ...
            optimizer=optimizer,
            logger=logger,
            loss_and_score=loss_and_score,
            g=g,
            args=args,
        )
        score_lst = [score_val, score_test]
        ...
        score_lst = [score_val, score_test_tran, score_test_ind]
    ...
    if args.save_results:
        # Save file
        # torch.save(g, f"graph_{args.dataset}.pt")
        # Splits and Indices
        # if args.glnn:
        #     # Loss curves
        #     loss_and_score = np.array(loss_and_score)
        #     np.savez(output_dir.joinpath("loss_and_score_glnn"), loss_and_score)
        #     # Model
        #     torch.save(model.state_dict(), output_dir.joinpath("model_glnn.pth"))
        # else:
        torch.save(distill_indices, output_dir.joinpath("idx_splits.pt"))
        # Loss curves
        loss_and_score = np.array(loss_and_score)
